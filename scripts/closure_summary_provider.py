...
def custom_closure_summary(value: lldb.SBValue, dict: dict[str, Any]) -> str:
    ...
    if value.GetByteSize() == 16:
        return print_as_swift_closure(value, process, address)
    elif value.GetByteSize() == 8:
        return print_assuming_preceeds_closure_ctx(value, process, address)
    else:
        return "<invalid size of " + str(value.GetByteSize()) + ">"

def print_as_swift_closure(value: lldb.SBValue, process: lldb.SBProcess, address: int) -> str:
...
def print_assuming_preceeds_closure_ctx(value: lldb.SBValue, process: lldb.SBProcess, address: int) -> str:
    fp: Optional[int] = read_qword(process, address)
